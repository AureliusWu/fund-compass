...
- 公开 V8 decision、outcome、policy、watchlist 和运行状态改用脱敏 DTO；完整私人读取只接受独立 `PRIVATE_READ_TOKEN`，Admin/Worker 身份不能替代。
...
- 匿名浏览器只得到脱敏 V8 DTO。可信 owner 会话/BFF 未完成前，首页、自选、详情中的私人动作面板明确显示不可用；长期 `PRIVATE_READ_TOKEN` 不下发浏览器。
