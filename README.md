...
> 说明：下文大写 `V8`—`V13` 是 2026 年 7 月的历史功能批次名，不是当前 SemVer；发布版本始终以本页顶部及应用内版本标记为准。
...
- 择时信号：估值、趋势、情绪三层信号。
- **决策卡片（v6.0.2）**：浏览器通过服务端代理批量读取东方财富估值表；非交易日自动降级为最近正式净值涨跌。详情决策由后端读取同一数据源；结合评分、估值、趋势、动量、回测和持仓状态，明确输出买入 / 分批定投 / 观望或加仓 / 持有 / 减仓 / 卖出，并展示强度、数据时间、风险与结论变化条件。
- **数据可靠性修复（v6.0.5）**：缺失净值/成本不再折算为 0；组合未完整覆盖时暂停盈亏、快照、权重和再平衡。历史回测不再读取当前指数估值，相关性使用共同日期上的日收益率，Worker 对每只基金独立校验新鲜度与数值完整性并回退正式净值。
- **运行真实性修复（v6.0.6）**：Cloudflare 推送调度按真实周一至周五运行，并分别记录自然调度、跳过原因、发送尝试和最近成功；SQLite 只有在数据库路径通过真实可写挂载点校验后才报告持久化。
- **盘中估值模型与健壮性（v7.0.0）**：参考蜉蝣基金的公开持仓穿透思路，在司南现有 Worker 契约内重写持仓模型；输出覆盖率、行情时间、模型状态和非官方标识。模型证据不足、行情过期或上游异常时明确降级为正式净值或不可用，不把推算值冒充基金公司净值。
- **受约束自迭代回测**：单基金训练/留出验证、跨基金聚合门槛和参数版本注册表；自动任务只生成 candidate、评估报告与晋级/回滚建议，`active/history` 只能由管理员显式治理。
