...
9. 缺失指标保持 `null` 并显示数据不足，不得转换成 0；评分总分必须满足至少 70% 覆盖率。
10. 缓存、指数估值和市场温度都必须携带新鲜度语义；上游全失败时不得生成新的中性观测。
11. 组合回测按共同净值日期对齐，不对缺失交易日做前向填充。
12. 基金经理等实体导航优先使用稳定公开 ID；提醒事件必须使用可复现指纹并迁移清理旧重复记录。
