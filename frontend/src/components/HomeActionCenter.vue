    ...
    <div class="action-layout">
      <section class="action-ledger" aria-label="V8 自选决策">
        <div class="distribution" aria-label="行动分布">
          <div class="do"><span>需要动作</span><b>{{ loading && !completed ? '—' : summary.action }}</b></div>
          <div class="hold"><span>继续持有</span><b>{{ loading && !completed ? '—' : summary.hold }}</b></div>
          <div class="watch"><span>观察</span><b>{{ loading && !completed ? '—' : summary.watch }}</b></div>
        ...
        <template v-else>
          ...
          <div v-if="errors.length" class="partial-note" role="alert">
            ...
            <span v-for="issue in errors" :key="issue.code">
              ...
              {{ issue.kind === 'missing' ? '尚无 V8 决策（404）' : 'V8 决策读取失败' }}
            </span>
          </div>
